...
| --------------- | ---------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `vcodec`        | `str`            | `"h264"`      | Recording CLI default. `"auto"` picks the first available hardware encoder from a fixed preference list, falling back to `libsvtav1`.                                                                            |
...
| `crf`           | `int` or `float` | `23`          | Recording CLI default. Abstract quality value, mapped per codec (see the [mapping](https://github.com/huggingface/lerobot/blob/main/src/lerobot/configs/video.py#L197)). Lower → higher quality / larger output. |
| `preset`        | `int` or `str`   | `"ultrafast"` | Recording CLI default. Encoder speed preset; meaning depends on the codec. When unset with `vcodec=libsvtav1`, LeRobot uses `12`.                                                                                |
...
These defaults apply to `DatasetRecordConfig` and the recording CLIs. Directly constructing
`RGBEncoderConfig()` keeps the generic AV1-oriented defaults for backward compatibility.
