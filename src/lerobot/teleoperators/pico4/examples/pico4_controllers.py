        ...
        while True:
            ...
            if i % 10 == 0:
                ...
                if read_times:
                    ...
                    print(
                        f"  SDK Read Latency:  avg={avg_read:.2f}ms  max={max_read_time:.2f}ms  current={read_time_ms:.2f}ms"
                    )
                if position_deltas:
                ...
                if current_time - last_zero_check_time >= zero_check_interval:
                    all_zero = (
                        all(abs(v) < 1e-6 for v in left_pose)
                        and all(abs(v) < 1e-6 for v in right_pose)
                        and all(abs(v) < 1e-6 for v in headset_pose)
                        and abs(left_trigger) < 1e-6
                        and abs(right_trigger) < 1e-6
                        and abs(left_grip) < 1e-6
                        and abs(right_grip) < 1e-6
                    )
                ...
                print(
                    f"  Position:    x={right_pose[0]:8.4f}  y={right_pose[1]:8.4f}  z={right_pose[2]:8.4f}"
                )
                print(
                ...
                print("\n[Inputs]")
                print(
                    f"  Left  Trigger: {left_trigger:6.3f}    Grip: {left_grip:6.3f}    X Button: {x_button}"
                )
                print(
                    f"  Right Trigger: {right_trigger:6.3f}    Grip: {right_grip:6.3f}    A Button: {a_button}"
                )
                ...
                if num_trackers > 0:
                    ...
                    for idx in range(min(num_trackers, 2)):  # Only show first 2 to save space
                        ...
                        print(
                            f"  Tracker {idx + 1} (SN: {sn}): x={pose[0]:6.3f} y={pose[1]:6.3f} z={pose[2]:6.3f}"
                        )
                else:
